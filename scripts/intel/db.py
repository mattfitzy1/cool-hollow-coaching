...
This workspace runs IntelOS against the shared DataOS database
(data/data.db), so meetings live alongside the rest of the business
data. The collection_log table is shared and has an identical schema.
"""
...
# Scripts live at scripts/intel/, so the workspace root is three levels up.
# IntelOS shares the existing DataOS database rather than making its own.
...
-- Collection log (tracks every collection run). Shared with DataOS;
-- schema is identical, so CREATE IF NOT EXISTS is a no-op when it exists.
