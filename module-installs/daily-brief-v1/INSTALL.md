...
> **Telegram / CommandOS retired (15 June 2026).** This module historically delivered the brief to a Telegram bot. CommandOS is decommissioned — partners and clients now use Remote Control. Brief delivery is moving to Slack or email via the rebuilt `brief.py`, which is still in progress. Until it lands, generate the brief and read it from `outputs/daily-brief/`. Do not wire Telegram.
...
> CommandOS (the Telegram bot) was retired on 15 June 2026. Brief delivery is moving to Slack or email via the rebuilt `brief.py` (in progress). Until it lands, the brief is generated and read from `outputs/daily-brief/` — no Telegram setup required.

[VERIFY] ContextOS, DataOS and IntelOS should be present. If any are missing, explain which module to install first and stop.
