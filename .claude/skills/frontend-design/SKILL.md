...
## This Workspace: the client's brand

Any frontend work in this workspace must hold the client's brand. Read the brand
source of truth before you start: **`context/brand.md`** (palette, type, feel) and
**`context/voice-and-tone.md`** (how the brand sounds). Match the brand exactly:

- **Palette:** use the colors defined in `context/brand.md`. Stay on the brand
  anchor ink and accent colors; do not introduce off-brand colors for depth.
- **Type:** use the typefaces defined in `context/brand.md` for wordmark, headlines,
  and body.
- **Feel:** match the brand's intended register (restraint vs energy, editorial vs
  bold). Pick the closest design example from the library below and adapt it to the
  brand.
- **Avoid:** any language or imagery the brand has explicitly ruled out (check
  `context/brand.md` for purged words and off-brand styles).

**Design system (note, do not run yet):** if the extraction step in Phase 2 below
has not been run for this brand's app, run Phase 2 once after the look is locked to
extract a `design-system/` folder (`tokens.css` + `components.css` + `DESIGN.md`) so
future plain-English edits stay on-brand automatically (the `app-design-system`
skill then enforces it). Until then, read the app's `index.html` and match its
existing CSS variables by hand. Do not run the extraction as part of an unrelated
task.
