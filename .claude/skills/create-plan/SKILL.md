---
name: create-plan
description: "Create a detailed implementation plan before making workspace changes"
---
...
- Create the plan in the `plans/` directory with filename: `YYYY-MM-DD-{descriptive-name}.md`
  - Use today's date
  - Replace `{descriptive-name}` with a short, kebab-case name (e.g., "add-restore-flow-to-app", "restructure-outputs", "create-content-batch-workflow")
...
2. **Explore relevant areas:**
   - If creating a skill or command: read existing skills in `.claude/skills/` and commands in `.claude/commands/`
