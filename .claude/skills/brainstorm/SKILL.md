---
name: brainstorm
description: "Scan workspace tasks, processes, and current setup to find the best automation opportunities to build next"
---
...
1. Read the workspace to understand the user's setup:
   - `CLAUDE.md` - what skills and commands are available, how the owner likes to work
   - The project register: `projects/README.md` and the files under `projects/active/` - what the owner is working on right now
   - The content pipeline: `content/strategy.md`, `content/pipeline.md`, `content/inbox.md`, and `outputs/social/` - what the owner is posting and what ideas are waiting
   - The context files (`context/business-info.md`, `context/strategy.md`, `context/brand.md`) for the owner's priorities and brand
2. If a specific topic was provided, focus on that area. If not, scan broadly.
3. Build a picture of:
   - **What's already set up** - skills, commands, and tools the owner has connected
   - **What's still manual or fiddly** - things the owner does by hand that take time or get skipped
   - **What's sitting unfinished** — drifting projects, captured ideas never developed, posts never made
   - **What tools the owner uses** - Canva, the scheduler, Cloudflare, the creative tools
...
4. Present a summary:
   ...
   Already set up:
   - {tool/skill 1} — {what it does for you}
   - {tool/skill 2} — {what it does for you}

   Opportunities (manual, fiddly, or unfinished):
   - {thing 1} — {how often, how much of a drag}
   - {thing 2} — {how often, how much of a drag}
   - {thing 3} — {how often, how much of a drag}

   Worth a look:
   - {ideas captured but never developed, projects drifting, posts not made}
   ```
...
- **Start from real work** - The best ideas come from what the owner is actually doing: active projects, the content ideas sitting in `content/inbox.md` that never got developed, posts they keep meaning to make, app edits they have mentioned. Look there first.
